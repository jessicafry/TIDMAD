...
10. Run `AxionPhoton_TIDMAD.ipynb` to produce the global Dark Matter Limit plot. This Jupyter notebook utilizes the plotting tools from [AxionLimits](https://github.com/cajohare/AxionLimits) along with a specific plotting function `PlotFuncs_TIDMAD.py` for this project.
       * **Note**: the raw and denoised `.csv` files generated by step 8 must be provided to the `ABRACADABRA_TIDMAD` function, i.e. `AxionPhoton.ABRACADABRA_TIDMAD("brazilBand_largedata_coarse.csv","brazil_all_FCNet_0_4.csv",ax,col='red',edgealpha=0)`
